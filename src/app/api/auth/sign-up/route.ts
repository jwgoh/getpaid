...
import { errorResponse, parseBody, withPublic } from "@app/server/api/route-helpers";
...
export const POST = withPublic(
  ...
  [
    {
      check: (error) => error instanceof EmailExistsError,
      respond: (error) => errorResponse("EMAIL_EXISTS", error.message, 409),
    },
  ]
);
