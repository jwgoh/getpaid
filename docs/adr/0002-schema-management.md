...
6. **`db:push:dev-only` script retained for local throwaway dev only.** `package.json` keeps `db:push:dev-only` (`prisma db push`) — it is a legitimate tool for fast schema iteration against a disposable local database where migration history does not matter. The script name carries the caveat: it must NEVER run against a non-dev or production database. Production and CI apply schema changes exclusively via `prisma migrate deploy`. `CLAUDE.md` ("Database migrations") documents `db push` on the same dev-only terms.
